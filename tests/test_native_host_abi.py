...
class NativeHostABITests(unittest.TestCase):
    ...
    def test_native_cmake_interface_configures_when_cmake_exists(self):
        if shutil.which("cmake") is None:
            self.skipTest("cmake is not installed")
        ...
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("APC native host runtime interface configured", completed.stdout + completed.stderr)

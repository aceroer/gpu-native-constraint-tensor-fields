...
This document records the procedure for a future public `0.3` tag.
...
artifact_commit: fill from /tmp/apc-release-artifacts-0-3.json commit
tag_kind: pending
```
...
print(json.loads(Path("/tmp/apc-release-artifacts-0-3.json").read_text())["commit"])
PY
```
...
```bash
git tag -a v0.3.0-alpha.N -m "v0.3.0-alpha.N"
...
CUDA smoke fails instead of skipping cleanly without CUDA tooling.
Public terminology boundary scan is not empty.
```

## Limits

The public `0.3` tag remains bounded:
